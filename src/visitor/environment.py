...
class Scope:
    ...
    def find_and_set_old_variable(self, name: str,  value: Value):  # todo, aby sprawdzać czy zmienny różnych typów o tej samej nazwie
        if name in self.variables.keys():  # two
            ...
            raise VariableNameConflictError(name, value.type, existing_variable_type)
        return False
...
class CallContext:
    ...
    def get_variable(self, variable_name: str) -> Value | None:
...
class Environment:
    def __init__(self) -> None:
        ...
        self.call_contexts = []  # [CallContext(Scope, Scope...), CallContext, ...]
